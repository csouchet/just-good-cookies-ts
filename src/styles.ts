...
export function loadBannerLayout(style: any) {
  ...
  const yesCookies5 = JGC.customStyle?.accept
    ? JGC.customStyle.accept
    : checkTailwindPrefix('text-white dark:text-green-400 px-5 text-sm text-center font-bold uppercase transition focus:shadow-2xl tracking-wide rounded-full border-none');
  ...
  const yesCookies7 = JGC.customStyle?.reject
    ? JGC.customStyle.reject
    : checkTailwindPrefix('text-gray-600 dark:text-white rounded-full text-xs font-semibold focus:shadow-2xl border-none');
  const noCookies7 = JGC.customStyle?.accept
    ? JGC.customStyle.accept
    : checkTailwindPrefix('text-gray-600 dark:text-white text-xs leading-6 font-bold focus:shadow-2xl rounded-full border-none');
        ...
        JGC.bannerConfig.backgroundImage ? `background-size:cover; background-image:url(${JGC.bannerConfig.backgroundImage})` : ''
            ...
            ${
              JGC.bannerConfig.backgroundImage
                ? `${JGC.bannerConfig.backgroundColor} ${checkTailwindPrefix('p-2')}`
                : `${JGC.bannerConfig.innerBackgroundImage ? '' : checkTailwindPrefix('p-6')}`
            }
            ...
            <div class="${checkTailwindPrefix('space-y-2 flex flex-col')} ${
              ...
              <div class="${checkTailwindPrefix('flex w-full')}" >
                  ...
                  ${
                    JGC.bannerConfig.closeButton
                      ? `<button id="jgc-close-button" class="${JGC.bannerConfig.logo ? `${checkTailwindPrefix('absolute top-2 right-4')}` : ``}   ${
                          JGC.customStyle?.closeButton ? `${JGC.customStyle.closeButton}` : `${checkTailwindPrefix('dark:text-white text-xl ml-auto m-0 p-0 bg-transparent')}`
                        }">&times;</button>`
                      : ''
                  }
              </div>
              <div class="${JGC.customStyle?.bannerText ? JGC.customStyle.bannerText : checkTailwindPrefix('dark:text-white')} ${
        JGC.bannerConfig.innerBackgroundImage ? checkTailwindPrefix('px-6') : ''
      }">
                ...
                <div>${
                  JGC.bannerLink
                    ...
                    : `${JGC.locale.bannerLinkDescription} <a class="${
                        JGC.customStyle?.privacyLink ?? `${checkTailwindPrefix('dark:decoration-sky-500 dark:underline font-bold text-black dark:text-white')}`
      ...
      class="${JGC.positions[JGC.bannerConfig.position || 'bottom']}
        ...
        ${getMaxWidth('max-w-5xl')}
          ${checkTailwindPrefix(
            ...
            ${
              JGC.bannerConfig.closeButton
                ? `<button id="jgc-close-button" class="${JGC.customStyle?.closeButton ? JGC.customStyle.closeButton : checkTailwindPrefix('text-white')} ${
                    JGC.customStyle?.closeButton ? JGC.customStyle.closeButton : checkTailwindPrefix('bg-black dark:bg-white')
                  }  
            ...
            &times;</button>`
              ...
              <div class="${JGC.customStyle?.bannerText ? JGC.customStyle.bannerText : `${checkTailwindPrefix('dark:text-gray-300')}`} ${checkTailwindPrefix(
        'text-sm text-center',
      )}">
                <div>${JGC.bannerText ? JGC.bannerText : `${JGC.text.descriptionText ? JGC.text.descriptionText : JGC.locale.bannerDescription}`} <br/></div>
                <div>${
                  JGC.bannerLink ??
                  `<div>${JGC.locale.bannerLinkDescription} <a class="${checkTailwindPrefix('font-bold')}" target="_blank" href="${JGC.config.privacyLink}"> ${
        ...
        JGC.bannerConfig.shortText ? JGC.locale.rejectShortText : JGC.text.rejectText
              ...
              <div class="${checkTailwindPrefix('mt-2')}">${
        // @ts-expect-error TS(2554): Expected 1 arguments, but got 0.
        managePreferencesLink()
      }</div>
            </div>
      ...
      ${checkTailwindPrefix('fixed shadow-2xl mx-auto md:left-1/2 md:-translate-x-1/2 md:mb-[1.5%]  transition duration-700 ease-in-out z-[999] md:rounded-full')}">
            ...
            ${JGC.bannerConfig.backgroundImage ? `${JGC.bannerConfig.backgroundColor} ${checkTailwindPrefix('p-6 rounded-full')}` : checkTailwindPrefix('p-4')}">
              ...
              <div class="${checkTailwindPrefix('col-span-2 flex items-center justify-center')}">
                ${
                  JGC.bannerConfig.closeButton
                    ? `<button id="jgc-close-button" class="${JGC.customStyle?.customStyle ? JGC.customStyle.customStyle : checkTailwindPrefix('text-white')} 
        ...
        JGC.bannerConfig.shortText ? JGC.locale.rejectShortText : JGC.text.rejectText
                ...
                <div class="${checkTailwindPrefix('mt-2 flex justify-center')}">${
        // @ts-expect-error TS(2554): Expected 1 arguments, but got 0.
        managePreferencesLink()
      }</div>
              </div>
      ...
      ${checkTailwindPrefix('w-full min-h-screen fixed flex flex-col shadow-2xl top-0 transition duration-700 ease-in-out z-[999]')}">
          ...
          JGC.bannerConfig.backgroundImage
            ...
            : ''
              ...
              ${checkTailwindPrefix('relative  flex flex-col justify-between items-center')}">
                ${JGC.bannerConfig.innerBackgroundImage ? `<img class="${checkTailwindPrefix('md:rounded-t')}" src="${JGC.bannerConfig.innerBackgroundImage}" />` : ''}
                <div class="${checkTailwindPrefix('flex flex-col justify-start w-full px-6')} ${
        JGC.bannerConfig.innerBackgroundImage ? checkTailwindPrefix('pb-6') : checkTailwindPrefix('py-6')
      } ${checkTailwindPrefix('space-x-1"')}>
                ${returnLogo()}
                ${
                  JGC.bannerConfig.closeButton
                    ? `<button id="jgc-close-button" class="${JGC.customStyle?.closeButton ? JGC.customStyle.closeButton : checkTailwindPrefix('text-white')} 
                ${JGC.customStyle?.closeButton ? JGC.customStyle.closeButton : checkTailwindPrefix('bg-black')} ${checkTailwindPrefix(
        ...
        'flex flex-col text-sm px-6 pb-6',
                  ...
                  ${
                    JGC.bannerLink
                      ? JGC.bannerLink
                      : `${JGC.locale.bannerLinkDescription} <a class="${checkTailwindPrefix('font-bold')}" target="_blank" href="${JGC.config.privacyLink}"> ${
                          JGC.text?.bannerLinkLabel ? JGC.text.bannerLinkLabel : JGC.locale.bannerLinkLabel
                        }</a>`
                  }  
                </div>  
      ...
      JGC.banner.innerHTML = `<div style="${JGC.bannerConfig.backgroundImage && `background-size:cover; background-image:url(${JGC.bannerConfig.backgroundImage})`}"
        ...
        ${
          JGC.bannerConfig.innerBackgroundImage
            ? `${checkTailwindPrefix('grid grid-cols-5')}  ${JGC.bannerConfig.backgroundImage ? '' : checkTailwindPrefix('gap-6')}`
            : checkTailwindPrefix('flex flex-col')
        }
        ...
        ${
          JGC.bannerConfig.closeButton
            ? `<button id="jgc-close-button" class="${JGC.customStyle?.closeButton ? JGC.customStyle.closeButton : checkTailwindPrefix('text-black dark:text-white')} 
            ...
            ${checkTailwindPrefix('flex flex-col items-start space-y-3')}">                  
                <div>
                  ...
                  <div class="${JGC.customStyle?.bannerText ? JGC.customStyle.bannerText : `${checkTailwindPrefix('dark:text-gray-300')}`}">
                    ...
                    <a class="${checkTailwindPrefix('font-bold')}" target="_blank" href="${JGC.config.privacyLink}"> ${
                            JGC.text?.bannerLinkLabel ? JGC.text.bannerLinkLabel : JGC.locale.bannerLinkLabel
                          }</a>.`
                    }  
                  </div>
      ...
      ${
        JGC.bannerConfig.backgroundImage
          ...
          : ''
            ...
            ${
              JGC.bannerConfig.closeButton
                ? `<button id="jgc-close-button" class="${checkTailwindPrefix('rounded-full w-6 h-6 self-end absolute top-4 right-6')} ${
                    JGC.customStyle?.closeButton ? JGC.customStyle.closeButton : checkTailwindPrefix('text-white m-0 p-0 bg-transparent')
                  }">&times;</button>`
                : ''
            } 
              ...
              <div class="${checkTailwindPrefix('flex justify-start w-full')} ${JGC.bannerConfig.innerBackgroundImage && checkTailwindPrefix('px-6')}">
                <div class="${checkTailwindPrefix('space-x-2 w-full flex')}">
                    <button type="button" role="button" tabindex="0" id="yesCookies" class="${yesCookies7} ${checkTailwindPrefix('group flex items-center')}">
                      ...
                      <div class="${JGC.customStyle?.accept ? JGC.customStyle.accept : checkTailwindPrefix('group-hover:text-sky-800 dark:group-hover:text-green-200')}">${
        JGC.bannerConfig.shortText ? JGC.locale.acceptShortText : JGC.text.acceptText
      }</div>
                    </button>
                    <button type="button" role="button" tabindex="0" id="noCookies" class="${noCookies7} ${checkTailwindPrefix('group flex items-center')}">
                      <div class="${checkTailwindPrefix(
                        ...
                        <div class="${JGC.customStyle?.reject ? JGC.customStyle.reject : checkTailwindPrefix('group-hover:text-sky-800 dark:group-hover:text-red-200')}">${
        JGC.bannerConfig.shortText ? JGC.locale.rejectShortText : JGC.text.rejectText
      }</div>
                      </button>
                    </div>
                    <div class="${checkTailwindPrefix('ml-auto w-full text-right')}">${
        // @ts-expect-error TS(2554): Expected 1 arguments, but got 0.
        managePreferencesLink()
      }</div>
                </div>
    ...
    case 'style8':
      ...
      JGC.banner.innerHTML = `<div style="${JGC.bannerConfig.backgroundImage && `background-size:cover; background-image:url(${JGC.bannerConfig.backgroundImage})`}"
            ...
            ${checkTailwindPrefix('flex flex-col items-start space-y-3 w-full')}">                  
                <div class="${checkTailwindPrefix('w-full')}">
                  <div class="${checkTailwindPrefix('flex items-center w-full mb-3')}" >
                    ...
                    ${
                      JGC.bannerConfig.closeButton
                        ? `<button id="jgc-close-button" class="${JGC.customStyle?.closeButton}  ${checkTailwindPrefix('ml-auto')}">&times;</button>`
                        : ''
                    }
                  </div>
                  <div class="${JGC.customStyle?.bannerText ? JGC.customStyle.bannerText : `${checkTailwindPrefix('dark:text-gray-300')}`} ${checkTailwindPrefix(
        ...
        JGC.bannerConfig.shortText ? JGC.locale.acceptSelectedShortText : JGC.text.acceptSelectedText
                  ...
                  <div class="${checkTailwindPrefix('group')}">
                    <div class="${JGC.customStyle?.reject ? JGC.customStyle.reject : checkTailwindPrefix('')} ${checkTailwindPrefix(
        'py-2 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 ease-linear',
                ...
                <div class="${checkTailwindPrefix('block w-full border-t md:hidden')} ">${
        // @ts-expect-error TS(2554): Expected 1 arguments, but got 0.
        managePreferencesLink()
      }</div>
              </div>
      ...
      JGC.banner.innerHTML = `<div id="bannerContent" style="${
        JGC.bannerConfig.backgroundImage ? `background-size:cover; background-image:url(${JGC.bannerConfig.backgroundImage})` : ''
                ...
                ${
                  JGC.bannerConfig.backgroundImage
                    ? `${JGC.bannerConfig.backgroundColor} ${checkTailwindPrefix('p-2')}`
                    : `${JGC.bannerConfig.innerBackgroundImage ? '' : checkTailwindPrefix('px-6 py-2')}`
                }
                ...
                <div class="${checkTailwindPrefix('flex items-center space-x-2')}
                  ...
                  <div class="${checkTailwindPrefix('col-span-3 flex space-x-2')}
                    ${JGC.customStyle?.bannerText ? JGC.customStyle.bannerText : checkTailwindPrefix('dark:text-white')} ${
        JGC.bannerConfig.innerBackgroundImage ? checkTailwindPrefix('px-6') : ''
      }">
                    <div>${
                      JGC.bannerText
